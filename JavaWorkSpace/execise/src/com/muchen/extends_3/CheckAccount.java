 ...
 * overdraft代表可透支限额。在CheckAccount类中重写withdraw方法，其算法如下:如果
 * （取款金额<账户余额)
 * 		可直接取款
 * 
 * 如果(取款金额>账户余额)，
...
public class CheckAccount extends Account{
	...
	public void withdraw(double amounr) {
		if(amounr <= getBalance()) {
			setBalance(getBalance() - amounr);
		}else {
			double over = (amounr - getBalance());
			if(overdraft >= over) {
				overdraft -= over;
				setBalance(0);
			}else {
				System.out.println("您已超过可透支额的限额！");
			}
		}
	}
